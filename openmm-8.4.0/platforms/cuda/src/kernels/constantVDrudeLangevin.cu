...
__device__ double blockReduceSum(double val) {
    // Fixed-size shared memory: 32 elements (max 32 warps = 1024 threads)
    // This is safe for all block sizes up to 1024 threads
    ...
    // Store warp-level sum in shared memory
    if (lane == 0 && wid < numWarps) {
        shared[wid] = val;
    }
    __syncthreads();

    // Second reduction: reduce across warps (only first warp participates)
    if (wid == 0) {
...
/**
 * Kernel 3a: Update buckyball conductor charges - Step 1 (Surface Polarization)
 * Corresponds to: MM_classes.py::Numerical_charge_Conductor() Lines 391-422
 * 
 * CRITICAL: This must be followed by force recalculation before Step 2!
 */
__global__ void updateBuckyballChargesStep1Kernel(
    ...
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < bucky.numAtoms; i += blockDim.x * gridDim.x) {
        ...
        // Update charge (matches MM_classes.py::Numerical_charge_Conductor() Line 412)
        // q_i = 2.0 / (4π) × area_atom × E_n_external × conversion_KjmolNm_Au
        double factor = 2.0 / FOUR_PI * CONVERSION_KJMOL_NM_TO_AU;
        double q_new = factor * bucky.area_atom * E_n_external;

        // Clamp to small threshold if needed (original line 417-418)
        if (fabs(q_new) < SMALL_THRESHOLD) {
            q_new = SMALL_THRESHOLD;  // Cathode, make positive
        }
...
/**
 * Kernel 3b: Update buckyball conductor charges - Step 2 (Charge Transfer)
 * Corresponds to: MM_classes.py::Numerical_charge_Conductor() Lines 429-495
 * 
 * CRITICAL: This must be called AFTER force recalculation (after Step 1)!
 */
__global__ void updateBuckyballChargesStep2Kernel(
    const BuckyballData* __restrict__ buckyballs,
    int buckyballIndex,
    const long long* __restrict__ force,
    float4* __restrict__ posq,
    ...
    // Corresponds to: MM_classes.py Lines 435-487
    // ═══════════════════════════════════════════════════════════════════════

    if (threadIdx.x == 0 && blockIdx.x == 0) {
        // Read force at contact electrode atom (original lines 436-452)
        // Contact atom is on the electrode (cathode/anode), NOT on the Buckyball
        int contactIdx = bucky.contactAtomIndex;
        
        // Bounds check for contact atom index
        if (contactIdx < 0 || contactIdx >= paddedNumAtoms) {
            // Invalid contact atom index, skip charge transfer
            dq_atom_shared = 0.0;
            return;
        }
        ...
        // Normal field at contact atom (original line 444-450)
        // For flat electrode, normal vector is (0, 0, ±1) depending on electrode type
        // Corresponds to: MM_classes.py L450: numpy.dot(E_external, [conductor_atom.nx, conductor_atom.ny, conductor_atom.nz])
        // For electrode atoms: nx=0, ny=0, nz=±1 (Fixed_Voltage_routines.py L266)
        double E_n_contact = 0.0;
        if (fabs(q_contact) > 0.9 * SMALL_THRESHOLD) {
            double Ex_contact = Fx_contact / q_contact;
            double Ey_contact = Fy_contact / q_contact;
            double Ez_contact = Fz_contact / q_contact;
        ...
        // Total charge transfer for spherical geometry (original line 473)
        double sign = -1.0;
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Add charge transfer to all atoms (original lines 490-495)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < bucky.numAtoms; i += blockDim.x * gridDim.x) {
        ...
        double q_surface = (double)posq[virtualIdx].w;  // From Step 1
        double q_total = q_surface + dq_atom_shared;  // Original line 493
...
/**
 * Kernel 3c: Update nanotube conductor charges - Step 1 (Surface Polarization)
 * Corresponds to: MM_classes.py::Numerical_charge_Conductor() Lines 391-422
 * 
 * CRITICAL: This must be followed by force recalculation before Step 2!
 */
__global__ void updateNanotubeChargesStep1Kernel(
    ...
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < tube.numAtoms; i += blockDim.x * gridDim.x) {
        ...
        // Clamp to small threshold if needed (original line 417-418)
        if (fabs(q_surface) < SMALL_THRESHOLD) {
            q_surface = SMALL_THRESHOLD;  // Cathode, make positive
        }

        posq[virtualIdx].w = (float)q_surface;
    }
}

/**
 * Kernel 3d: Update nanotube conductor charges - Step 2 (Charge Transfer)
 * Corresponds to: MM_classes.py::Numerical_charge_Conductor() Lines 429-495
 * 
 * CRITICAL: This must be called AFTER force recalculation (after Step 1)!
 */
__global__ void updateNanotubeChargesStep2Kernel(
    const NanotubeData* __restrict__ nanotubes,
    int nanotubeIndex,
    const long long* __restrict__ force,
    float4* __restrict__ posq,
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        // FIX: Normal field at contact atom using actual normal vector (original line 450)
        // Original: En_external = numpy.dot(E_external, [conductor_atom.nx, conductor_atom.ny, conductor_atom.nz])
        double E_n_contact = 0.0;
        if (fabs(q_contact) > 0.9 * SMALL_THRESHOLD) {
            // Project force onto contact normal vector
            double Ex_contact = Fx_contact / q_contact;
            double Ey_contact = Fy_contact / q_contact;
            double Ez_contact = Fz_contact / q_contact;
...
extern "C" void executeConstantVDrudeLangevinStep(
    ...
    for (int iter = 0; iter < scfIterations; iter++) {
        ...
        // FIX: Use dynamic grid size based on nanotube atom count for better GPU utilization
        if (numNanotubes > 0) {
            for (int tubeIdx = 0; tubeIdx < numNanotubes; tubeIdx++) {
                // Get nanotube atom count (need to read from device)
                // For now, use a reasonable grid size - kernel uses grid-stride loop
                // Typical nanotube: 100-1000 atoms, so use multiple blocks
                int blockSize = 256;
                // Estimate: assume average 500 atoms per nanotube, use 2-4 blocks
                // Kernel uses grid-stride loop, so this is safe even if estimate is wrong
                int numBlocks = 4;  // Use 4 blocks for better parallelism
...
/**
 * MD Integration Only (after SCF)
 * 
 * This function handles only the Drude Langevin integration part,
 * after SCF charge updates have been completed.
 */
