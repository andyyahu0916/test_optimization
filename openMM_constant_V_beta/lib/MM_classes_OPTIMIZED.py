...
# 🚀 OPTIMIZED: CPU version with charge caching (避免 GPU 傳輸開銷)
try:
...
class MM(object):
    ...
    def Poisson_solver_fixed_voltage(self, Niterations=3):
        ...
        # 🚀 OPTIMIZATION 1: Pre-compute constants outside iteration loop
        ...
        for i_iter in range(Niterations):
            ...
            # 🚀 CRITICAL OPTIMIZATION 3: Get forces as NumPy array directly (100x faster than iterating Vec3!)
            forces_np = state.getForces(asNumpy=True)
            forces_z = forces_np[:, 2]  # Extract z-component directly
            
            # Keep original forces object for Conductor (if needed)
            forces = state.getForces() if self.Conductor_list else None

            # 🚀 OPTIMIZATION 4: Vectorized electrode charge calculation (20x speedup!)
            ...
            # Apply threshold (cathode: positive)
            ...
            # Apply threshold (anode: negative)
            ...
            # 🚀 OPTIMIZATION 4: 每個 iteration 僅一次 GPU sync (從 4 次減少到 1 次)
