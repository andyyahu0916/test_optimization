...
double CudaCalcElectrodeChargeKernel::execute(
    ...
    // OPTIMIZATION: Use persistent device memory (avoid repeated allocation)
    ...
    // Upload forces and positions (reuse existing buffers)
    ...
    // Upload to persistent buffers (no allocation overhead)
    ...
    // Step 2: Compute electrode charges
    updateElectrodeCharges<<<numBlocksCathode, threadsPerBlock>>>(
        (const float4*)forcesDevicePersistent->getDevicePointer(),
        (const float4*)posqDevicePersistent->getDevicePointer(),
        (const int*)cathodeIndicesDevice->getDevicePointer(),
        parameters.cathodeIndices.size(),
        cathodeArea,
