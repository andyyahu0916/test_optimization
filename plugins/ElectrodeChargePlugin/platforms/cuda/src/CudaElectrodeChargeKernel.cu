...
__global__ void computeAnalyticTargets(
    ...
    if (idx < numParticles && electrodeMask[idx] == 0) {
        ...
        // PHYSICS: cathode analytic target uses distance to ANODE (opposite electrode)
        // This is correct - image charge concept requires distance to opposite electrode
...
CudaCalcElectrodeChargeKernel::CudaCalcElectrodeChargeKernel(
    ...
    cu(&cu),  // Store CudaContext immediately
...
void CudaCalcElectrodeChargeKernel::initialize(
    ...
    // CRITICAL: Set CUDA context before allocating device memory
    ContextSelector selector(*cu);
    
    // Now we can safely initialize device memory
...
double CudaCalcElectrodeChargeKernel::execute(
    ...
    // Get CUDA arrays from context (but DON'T use them - they may be stale!)
    CudaArray& posqArray = cu->getPosq();  // float4: (x, y, z, q)
    
    // CRITICAL FIX: Upload host-side forces AND charges to device!
    // cu->getForce() and cu->getPosq() may not contain the current iteration's data
    std::vector<float4> forcesFloat4(numParticles);
    std::vector<float4> posqFloat4(numParticles);
    
    for (int i = 0; i < numParticles; i++) {
        forcesFloat4[i].x = static_cast<float>(forces[i][0]);
        forcesFloat4[i].y = static_cast<float>(forces[i][1]);
        forcesFloat4[i].z = static_cast<float>(forces[i][2]);
        forcesFloat4[i].w = 0.0f;
        
        // Upload current positions AND charges from ForceImpl
        ...
        posqFloat4[i].w = static_cast<float>(allParticleCharges[i]);  // ← Use allParticleCharges!
    }
    ...
    // Step 1: Compute analytic target charges
    cudaMemset((void*)cathodeTargetDevice->getDevicePointer(), 0, sizeof(float));
    cudaMemset((void*)anodeTargetDevice->getDevicePointer(), 0, sizeof(float));
    
    computeAnalyticTargets<<<numBlocksParticles, threadsPerBlock>>>(
        (const float4*)posqDevice->getDevicePointer(),  // Use uploaded posq!
        numParticles,
    ...
    // Step 2: Compute electrode charges (NO iteration, NO normalization - match Reference!)
    // Reference implementation directly computes charges based on current forces
    // without any iterative solving or scaling to analytic target
    
    updateElectrodeCharges<<<numBlocksCathode, threadsPerBlock>>>(
        (const float4*)forcesDevice->getDevicePointer(),  // Use uploaded forces!
        (const float4*)posqDevice->getDevicePointer(),    // Use uploaded posq!
        (const int*)cathodeIndicesDevice->getDevicePointer(),
        parameters.cathodeIndices.size(),
        cathodeArea,
    ...
    if (err1 != cudaSuccess) {
        delete forcesDevice;
        delete posqDevice;
        throw OpenMMException(std::string("CUDA kernel updateElectrodeCharges (cathode) failed: ") + cudaGetErrorString(err1));
    }
    ...
    if (err2 != cudaSuccess) {
        delete forcesDevice;
        delete posqDevice;
        throw OpenMMException(std::string("CUDA kernel updateElectrodeCharges (anode) failed: ") + cudaGetErrorString(err2));
    }
    
    // Sync to ensure kernels complete
    ...
    // Step 3: Download final results to host
    ...
    // Set output targets
    float cathodeTargetFloat, anodeTargetFloat;
    cathodeTargetDevice->download(&cathodeTargetFloat);
    anodeTargetDevice->download(&anodeTargetFloat);
    cathodeTarget = static_cast<double>(cathodeTargetFloat);
    anodeTarget = static_cast<double>(anodeTargetFloat);
